...
namespace Carppi
{
    ...
    public class MainActivity : AppCompatActivity
    {
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
           ...
           switch(IsInWebDatabase())
            {
                case ResponseToExistanceInDbRequest.Existance:
                    {
                    ...
                    break;
                case ResponseToExistanceInDbRequest.Nonexistance:
                    {
                        try
                        {
                            ...
                            if (isLoggedIn() && (query.RegistroValidado != null && query.VerificacionDecuenta != null) && (query.RegistroValidado == true && query.VerificacionDecuenta == true))
                            {
                                Static_Activity = this;
                                static_FragmentMAnager = SupportFragmentManager;
                            ...
                            else if (isLoggedIn() && (query.RegistroValidado == true && query.VerificacionDecuenta != true))
                            {
                                Intent intento = new Intent(this, typeof(ValidateAccountActivity));
                    ...
                    break;
                case ResponseToExistanceInDbRequest.ExistanceWithoutValidity:
                    {
                        Intent intento = new Intent(this, typeof(ValidateAccountActivity));
                        StartActivity(intento);
                    }
        ...
        public enum ResponseToExistanceInDbRequest { Nonexistance, Existance, ExistanceWithoutValidity};
        public ResponseToExistanceInDbRequest IsInWebDatabase()
        {
            try
            {
                ...
                var TokeNData = PostLoginActivityActivity.SuperToken == null ? query.FirebaseID : PostLoginActivityActivity.SuperToken;


                var uri = new Uri(string.Format("http://geolocale.azurewebsites.net/api/CarppiRestaurantApi/SeekForHashInService?" +
                  "CarppiHashToSeek=" + query.CarppiHash +
                  "&FirebaseTag=" + TokeNData
                ...
                {
                    return ResponseToExistanceInDbRequest.Existance;
                }
                else if (S_Ressult.httpStatusCode == System.Net.HttpStatusCode.Moved)
                {
                    return ResponseToExistanceInDbRequest.ExistanceWithoutValidity;
                }
                else
                {
                    return ResponseToExistanceInDbRequest.Nonexistance;
                }
        ...
        {
            Intent intento = new Intent(this,typeof( LoginActivity));
            StartActivity(intento);
        ...
        */
        ...
        public class BottomCalback : BottomSheetBehavior.BottomSheetCallback
        {
            ...
            public override void OnStateChanged(View bottomSheet, int newState)
            {
               ...
               if(PreviousState  == BottomSheetBehavior.StateHalfExpanded && newState == BottomSheetBehavior.StateHidden )
                {
                    CancelAlProposalls();
            ...
            public async void CancelAlProposalls()
            {
                ...
                var databasePath5 = System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments), "RestaurantLogData.db");
                var db5 = new SQLiteConnection(databasePath5);
                var query = db5.Table<DatabaseTypes.RestauratLoginTypes>().Where(v => v.ID == 1).FirstOrDefault();

                var uri = new Uri(string.Format("http://geolocale.azurewebsites.net/api/TravelerCrossCityApi/CancellProposals?" +
                    "user9_Hijo=" + query.FacebookId
        ...
        protected override void OnActivityResult(int requestCode, Result resultCode, Android.Content.Intent data)
        {
            ...
            else if ((requestCode == (int)WebInterfaceAddProduct.ActivityIntentREturns.SelectEcomercePicture) && (resultCode == Result.Ok) && (data != null))
            {
                Android.Net.Uri uri = data.Data;
                Stream iStream = ContentResolver.OpenInputStream(uri);
                byte[] inputData = getBytes(iStream);
        ...
        void LoadFragment(int id)
        {
            ...
            switch (id)
            {
                ...
                case Resource.Id.menu_video:
                    ...
                    fragment = FragmentAddElement.NewInstance();
                    break;
...
;
        ...
        public static async Task LoggInWebTask()
        {
            ...
            try
            {
                ...
                var uri = new Uri(string.Format("http://geolocale.azurewebsites.net/api/CarppiRepartidorApi/RegisterUserProfile?" +
                    "Face_identifier_2=" + query.FacebookId + //VistaHTMLProffesores.Grupo_Activo +
        ...
        async void SendRegistrationToServerAsync(string token)
        {
            ...
            var databasePath5 = System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments), "RestaurantLogData.db");
            var db5 = new SQLiteConnection(databasePath5);
            var query_log = db5.Table<DatabaseTypes.RestauratLoginTypes>().Where(v => v.ID == 1).FirstOrDefault();
            HttpClient client = new HttpClient();
            //Post_Travel(string Argument, string FaceId, string Vehiculo, string Costo)


            var uri = new Uri(string.Format("http://geolocale.azurewebsites.net/api/TravelerCrossCityApi/UpdateFirebaseToken?" +
                "FaceID=" + query_log.FacebookId +//VistaHTMLProffesores.Grupo_Activo + Trip_Id
                "&FirebaseID=" + token
